...
class Gui:
    ...
    def password_receive(self):
        ...
        cur.execute("select password from student where student_id = '"+str(self.input_list[0])+"'")
        ...
        try:
            if str(pw[0])[14:-2] == str(self.input_list[1]):
                shop_id = []
                cur = self.conn.cursor()
                cur.execute("select student_id from student where student_id = '" + str(self.input_list[0]) + "'")
                for row in cur.fetchall():
                    shop_id.append(row)
                    print(shop_id)
                    print("test"+str(shop_id[0])[15:-1])
                vin3_template_3_fyp.main((str(shop_id[0])[15:-1]),"Student",self.root)
            else:
                self.wrong_()
    ...
    def __init__(self, root):
        ...
        self.name_label5 = tk.Label(self.frameL5, text="Student", font=("Helvetica", 30, "bold italic"), fg="white",background="black")
        ...
        self.db = pymysql.connect('localhost', 'root', '', 'fyp')
